...
class BuyAndHoldRebalance(Strategy):
    """
    ...
    def __init__(self, weights: np.ndarray = None):
        """Initialize BuyAndHoldRebalance strategy"""
        super().__init__("BuyAndHoldRebalance")
    ...
    def rebalance(self, date: pd.Timestamp, prices: np.ndarray, capital: float, current_positions: np.ndarray, *args, **kwargs) -> np.ndarray:
        ...
        -----------
        date : pd.Timestamp
            Current date (not used in this strategy but provided for consistency)
        ...
        if not np.all(current_positions == 0):
