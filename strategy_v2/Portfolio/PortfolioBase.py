...
class PortfolioBase(PortfolioModel, Performance):
    ...
    def backtest(self):
        '''
            Generate position and return based on optimized weights      

            In basetest, position are the controlled variables and position_shs depends on position
        '''        
        for i, system in enumerate(self.systems):
        ...
        assert(np.abs(port_ret2 - self.port_ret) >= 1e-5).sum() == 0, 'portfolio returns has discrepancy'
        ...
        daily_ret_dp = (self.port_position_shs * close_ret_dp).sum(axis=1).cumsum()

        # pct_change() is missing the first day return, so need to assign it manually
        port_ret3 = (self.capital + daily_ret_dp).pct_change().fillna(0)
        port_ret3.loc[self.start_date] = daily_ret_dp.iloc[0] / self.capital

        assert (np.abs(port_ret3 - self.port_ret) >= 1e-5).sum() == 0, 'portfolio returns has discrepancy'
        ...
        if self.tc_model is not None:
            ...
            self.port_capital = (self.capital + daily_ret_dp - port_rc_cum)      

            port_ret4 = self.port_capital.pct_change().fillna(0)            
            port_ret4.loc[self.start_date] = (daily_ret_dp - port_tc).iloc[0] / self.capital
    ...
    def rebalance(self):
        '''
            ...
            In rebalance, the position_shs are the controlled variables, where the position depends on position_shs
        '''        
        ...
        self.port_position_dp_rebal_strike = self.port_position_shs_rebal * self.close_px
        ...
        # regenerate the returns        
        close_ret_dp = self.close_px_raw.diff().loc[self.start_date:self.end_date]
        daily_ret_dp = (self.port_position_shs_rebal * close_ret_dp).sum(axis=1).cumsum()

        self.port_capital_rebal = self.capital + daily_ret_dp        
        self.port_ret_rebal = (self.port_capital_rebal).pct_change().fillna(0)
        self.port_ret_rebal.loc[self.start_date] = daily_ret_dp.iloc[0] / self.capital

        # derive the position in weights and capital
        capital_lag = self.port_capital_rebal.shift(1).fillna(self.capital)
        close_px_lag = self.close_px_raw.shift(1).loc[self.start_date:self.end_date]
        self.port_position_rebal = (self.port_position_shs_rebal * close_px_lag).divide(capital_lag, axis=0)        

        # re-derive the port_return again using % of return to double check
        close_ret = self.close_px_raw.pct_change().fillna(0)
        close_ret = close_ret.loc[self.start_date:self.end_date]
        port_ret2 = (self.port_position_rebal * close_ret).sum(axis=1)
        assert (np.abs(port_ret2 - self.port_ret_rebal) >= 1e-5).sum() == 0, 'rebalanced portfolio returns has discrepancy'
        ...
        if self.tc_model is not None:
            ...
            self.port_capital_rebal = (self.capital + daily_ret_dp - port_rc_cum)
            port_ret3 = self.port_capital_rebal.pct_change().fillna(0)            
            port_ret3.loc[self.start_date] = (daily_ret_dp - port_tc).iloc[0] / self.capital
